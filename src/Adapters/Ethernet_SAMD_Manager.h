...
const char ETM_HTML_HEAD[]     /*PROGMEM*/ = "<!DOCTYPE html><html><head><title>Ethernet_SAMD_Manager</title><style>div,input{padding:2px;font-size:1em;}input{width:95%;}\
...
#define ETHERNET_MANAGER_EEPROM_START    (EEPROM_START + DRD_FLAG_DATA_SIZE)
    ...
    bool EEPROM_get()
    {
      ...
      uint8_t* _pointer = (uint8_t *) &Ethernet_Manager_config;
      
      for (int i = 0; i < sizeof(Ethernet_Manager_config); i++, _pointer++, offset++)
      {              
        *_pointer = EEPROM.read(offset);
      }
      ...
      if (server)
      {
        ...
        if (key == "" && value == "")
        {
          ...
          if ( RFC952_hostname[0] != 0 )
          {
            ...
            result.replace("Ethernet_SAMD_Manager", RFC952_hostname);
          }
          else if ( Ethernet_Manager_config.board_name[0] != 0 )
          {
            // Or replace only if board_name is valid.  Otherwise, keep intact
            result.replace("Ethernet_SAMD_Manager", Ethernet_Manager_config.board_name);
          }
